...
class ActionCalculateEmissions(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        try:
            ...
            standardized_distance_unit = distance_unit_mapping.get(distance_unit.lower())
            if standardized_distance_unit not in valid_distance_units:
                dispatcher.utter_message(
            ...
            parameters["distance_unit"] = standardized_distance_unit

            # Get slots for money and money_unit
            money = tracker.get_slot("money")
            money_unit = tracker.get_slot("money_unit")

            if money:
                if not money_unit:
                    dispatcher.utter_message(text="Please provide the unit of money (e.g., USD, EUR).")
                    return []

                standardized_money_unit = money_unit_mapping.get(money_unit.lower())
                if standardized_money_unit not in valid_money_units:
                    dispatcher.utter_message(
                        text=f"Invalid money unit '{money_unit}'. Please use one of the following: {', '.join(valid_money_units)}."
                    )
                    return []

                parameters["money"] = money
                parameters["money_unit"] = standardized_money_unit
